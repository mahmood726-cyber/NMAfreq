...
instructionsUI <- function(id) {
  ...
  tagList(
    ...
    p("This app supports two types of data formats for network meta-analysis (NMA):"),
    tags$ul(
...
instructionsServer <- function(id) {
  moduleServer(id, function(input, output, session) {
    # No additional server logic required.
  })
}
...
dataUploadUI <- function(id) {
  ...
  tagList(
    ...
    checkboxInput(ns("logTrans"), "Log-transform outcome", FALSE),
...
dataUploadServer <- function(id) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$file, {
      ...
      if (input$format == "arm") {
        req_cols <- c("study", "treatment", "mean", "sd", "n", "covariate")
...
Study1,DrugA,DrugB,0.5,0.1,1.0,2010
Study2,DrugA,DrugC,0.3,0.12,1.2,2011
Study3,DrugB,DrugC,0.2,0.15,1.5,2012
Study4,DrugA,DrugB,0.55,0.11,1.0,2013
Study5,DrugC,DrugB,-0.1,0.2,1.3,2014
Study6,DrugA,DrugC,0.35,0.13,1.1,2015"
    
    sampleArmCSV1 <- "study,treatment,mean,sd,n,covariate,year
Study1,DrugA,2.5,0.5,30,1.0,2010
Study1,DrugB,1.8,0.6,30,1.0,2010
Study2,DrugA,3.0,0.7,25,1.5,2011
Study2,DrugB,2.7,0.6,25,1.5,2011
Study2,DrugC,2.9,0.8,25,1.5,2011
Study3,DrugA,4.1,1.1,40,2.0,2012"
    
    sampleContrastCSV2 <- "studlab,treat1,treat2,TE,seTE,covariate,covariate2,year
Study1,DrugA,DrugB,0.5,0.1,1.0,0.5,2010
Study2,DrugA,DrugC,0.3,0.12,1.2,0.7,2011
Study3,DrugB,DrugC,0.2,0.15,1.5,0.6,2012
Study4,DrugA,DrugB,0.55,0.11,1.0,0.5,2013
Study5,DrugC,DrugB,-0.1,0.2,1.3,0.8,2014
Study6,DrugA,DrugC,0.35,0.13,1.1,0.9,2015"
  ...
  moduleServer(id, function(input, output, session) {
    ...
    output$networkPlot <- renderVisNetwork({
      ...
      if (is.null(net) || nrow(net$nodes) == 0) {
        emptyNodes <- data.frame(id = "No data", label = "No data", title = "No data", stringsAsFactors = FALSE)
        emptyEdges <- data.frame(from = character(0), to = character(0), stringsAsFactors = FALSE)
        return(visNetwork(emptyNodes, emptyEdges, height = "500px", width = "100%"))
      }
...
analysisServer <- function(id, data) {
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      req(data())
      d_all <- data()
      mandatory <- c("study", "treatment", "mean", "sd", "n",
...
resultsUI <- function(id) {
  ...
  tabsetPanel(
    ...
    tabPanel("Meta-regression",
             # This tab shows meta-regression outputs (with multiple covariates allowed)
    ...
    tabPanel("Bubble Plot",
             plotOutput(ns("bubblePlot")) %>% withSpinner(),
             br(),
             downloadButton(ns("dlBubblePlotPNG"), "Download Bubble Plot as PNG")
    ),
    tabPanel("Contour Funnel Plot",
             fluidRow(
               column(6, textInput(ns("contourFunnelTitle"), "Plot Title:", value = "Contour Funnel Plot"))
             ),
             plotOutput(ns("contourFunnelPlot")) %>% withSpinner(),
             br(),
             downloadButton(ns("dlContourFunnelPNG"), "Download Contour Funnel Plot as PNG")
    ),
    tabPanel("Galbraith Plot",
             plotOutput(ns("galbraithPlot")) %>% withSpinner(),
             br(),
             downloadButton(ns("dlGalbraithPNG"), "Download Galbraith Plot as PNG")
    ),
    tabPanel("Sensitivity Plot",
             fluidRow(
               column(6, textInput(ns("sensPlotTitle"), "Plot Title:", value = "Sensitivity Analysis"))
             ),
             plotOutput(ns("sensitivityPlot")) %>% withSpinner(),
             br(),
             downloadButton(ns("dlSensPlotPNG"), "Download Sensitivity Plot as PNG")
    ),
...
resultsServer <- function(id, analysisResult) {
  moduleServer(id, function(input, output, session) {
    ...
    output$metaResidualPlot <- renderPlot({
      req(analysisResult())
      res <- analysisResult()
      if (!is.null(res$metareg)) {
        # Attempt to extract residuals and fitted values; assume meta-regression object supports them
        resid <- residuals(res$metareg)
        fit <- fitted(res$metareg)
        plot(fit, resid, xlab = "Fitted Values", ylab = "Residuals", main = "Meta-regression Residual Plot", pch = 16, col = "blue")
    ...
    output$bubblePlot <- renderPlot({
      ...
      res <- analysisResult()
      dat <- res$model$data
      if (!is.null(dat) && all(c("TE", "seTE") %in% names(dat))) {
        bubbleSize <- if("n" %in% names(dat)) dat$n else rep(10, nrow(dat))
        symbols(dat$TE, dat$seTE, circles = sqrt(bubbleSize), inches = 0.3,
                bg = "lightblue", fg = "black",
                xlab = "Treatment Effect", ylab = "Standard Error",
                main = "Bubble Plot of Effect Sizes")
        text(dat$TE, dat$seTE, labels = dat$studlab, pos = 3, cex = 0.8)
      } else {
        plot.new()
        text(0.5, 0.5, "Insufficient data for bubble plot.")
      }
    })
    output$dlBubblePlotPNG <- downloadHandler(
      filename = function() { "BubblePlot.png" },
      content = function(file) {
        png(file, width = 800, height = 600, units = "px", res = 96)
        req(analysisResult())
        res <- analysisResult()
    ...
    output$contourFunnelPlot <- renderPlot({
      req(analysisResult())
      res <- analysisResult()
      if(res$model$method == "freq" &&
         !is.null(res$model$trts) && length(res$model$trts) >= 2) {
        funnel(res$model, order = res$model$trts, main = input$contourFunnelTitle, 
               xlab = "Treatment Effect", ylab = "Standard Error")
        mtext("Contours at p=0.10, 0.05, 0.01", side = 3)
      } else {
        plot.new()
        text(0.5, 0.5, "Contour funnel plot not available.", cex = 1.3)
      }
    })
    output$dlContourFunnelPNG <- downloadHandler(
      filename = function() { "ContourFunnelPlot.png" },
      content = function(file) {
        png(file, width = 800, height = 600, units = "px", res = 96)
        res <- analysisResult()
        if(res$model$method == "freq" &&
    ...
    output$galbraithPlot <- renderPlot({
      req(analysisResult())
      res <- analysisResult()
      dat <- res$model$data
      if(!is.null(dat) && all(c("TE", "seTE") %in% names(dat))) {
        z <- dat$TE / dat$seTE
        prec <- 1 / dat$seTE
        plot(prec, z, xlab = "Precision (1/SE)", ylab = "Z value", main = "Galbraith Plot")
        abline(h = 0, col = "red", lty = 2)
      } else {
        plot.new()
        text(0.5, 0.5, "Insufficient data for Galbraith plot.", cex = 1.3)
      }
    })
    output$dlGalbraithPNG <- downloadHandler(
      filename = function() { "GalbraithPlot.png" },
      content = function(file) {
        png(file, width = 800, height = 600, units = "px", res = 96)
        req(analysisResult())
        res <- analysisResult()
    ...
    output$sensitivityPlot <- renderPlot({
      ...
      if (exists("leave1out", envir = asNamespace("netmeta"))) {
        loo <- netmeta:::leave1out(res$model)
        if(is.data.frame(loo) && nrow(loo) > 0 && "TE" %in% names(loo)) {
          plot(1:nrow(loo), loo$TE, type = "b", xlab = "Study Removed", ylab = "Effect Estimate", main = input$sensPlotTitle, xaxt="n")
          axis(1, at = 1:nrow(loo), labels = rownames(loo))
        } else {
          plot.new()
          text(0.5, 0.5, "Leave-one-out sensitivity plot not available.", cex = 1.3)
        }
      } else {
        plot.new()
        text(0.5, 0.5, "Leave-one-out function not available.", cex = 1.3)
      }
    })
    output$dlSensPlotPNG <- downloadHandler(
      filename = function() { "SensitivityPlot.png" },
      content = function(file) {
        png(file, width = 800, height = 600, units = "px", res = 96)
        req(analysisResult())
        res <- analysisResult()
...
# ---------------------------
ui <- bs4DashPage(
  title = "786-MIII NMA Shiny App (Frequentist Only - SMD/MD with Optional Meta-regression)",
